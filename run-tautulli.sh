...
if [ -f "/config/plexpy.ini" ] && [ ! -f "/config/tautulli.ini" ]; then
...
exec runuser -l tautulli -c '/opt/tautulli/app/PlexPy.py --nolaunch --quiet --config /config/tautulli.ini --datadir /config/data'
